...
function readField(
  ...
  return {
    ...
    isReadOnly: producedAtWrite(field),
...
/**
 * The value is produced when the row is written, rather than chosen by whoever
 * fills the form.
 *
...
function producedAtWrite(field: DmmfField): boolean {
  ...
  return field.hasDefaultValue && isFunctionDefault(field.default);
}

/**
 * `now()`, `autoincrement()`, `uuid()`, `cuid()`, `dbgenerated("…")` arrive as
 * `{ name, args }`; a default somebody could have typed arrives as the literal
 * `true`, `0` or `"draft"`. Who evaluates it is not the test and would be the
 * wrong one: `@default(true)` is a real column default, and `@default(uuid())`
 * is generated by the Prisma client with no column default at all.
 */
